...
- **Delete Sessions**: Press `d` or `Delete`, or use the visible Delete button, confirm the modal, and the app removes the session from your local Codex data before refreshing the list.
- **Dual-Pane Keyboard Navigation**: Press `Right` or `l` from the list to focus the Open in Codex button, then press `Enter` to activate it or `Left` / `h` to return to the list.
...
- **`Right`** or **`l`**: Move from the conversation list to the Open in Codex button, and from Open in Codex to Delete.
- **`Left`** or **`h`**: Move from Delete to Open in Codex, and from Open in Codex back to the conversation list.
